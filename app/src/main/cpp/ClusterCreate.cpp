//
// Created by karan on 18-05-2020.
//
...
struct params_server_work{
    ...
    std::vector<std::string> *pending_files;
    std::vector<std::string> *completed_files;
    std::set<unsigned int> *priority_table;
    std::map<int, client_info> *next_servers;
};
...
void get_pending_files(std::vector<std::string> *pending_files){
    DIR *directory;
    struct dirent *current;
    directory = opendir("../so_files/");
    if(directory){
        while((current = readdir(directory))!=NULL){
            std::string s(current->d_name);
            if(s.length()>3 && (s.compare(s.length()-3, 3, ".so") == 0) )
                pending_files->push_back(current->d_name);
        }
    }
    else{
        log_output("The .so file directory doesn't exist!\n");
    }
}


void recv_file(std::string pathstr ,client_info& client){
...
void send_file(std::string pathstr, client_info& client){
    char* path = (char*)malloc(sizeof(char)*pathstr.length());
    strcpy(path, pathstr.c_str());
...
void *connection_handler(void *socket_desc){
    ...
    std::map<int, client_info> *client_table = p->client_table;
    std::map<int, std::string> *work_table = p->work_table;
    std::vector<std::string> *pending_files = p->pending_files;
    std::vector<std::string> *completed_files = p->completed_files;
    std::map<int, client_info> *next_servers = p->next_servers;
    int sock = p->sd;
    int key = p->key;
    ...
    if(replicate){
        ...
        for(auto x: files){
            send_file((std::string("../so_files/")+x), (*client_table)[key]);
        }
    }
    ...
    while(1){
        if(work_table->find(key)==work_table->end()){
            log_output("No work assigned to client %d as of yet, going to sleep!\n", key);
        ...
        log_output("Client %d has been woken up!\n", key);
        //sleep(1);
        write(sock , "ping" , 5);
        ...
        write(sock , "file" , 5);
        send_file((std::string("../so_files/")+(*work_table)[key]), (*client_table)[key] );
        recv_file( (std::string("../op_files/")+"op"+ (*work_table)[key].substr(0, (*work_table)[key].length()-3)+".txt"), (*client_table)[key]);
        read_size = recv(sock , client_message , 2000 , 0);
        if((strcmp(client_message,"complete")!=0 || read_size==0 || read_size==-1)){ //the files hasn't been sent properly back to server
            log_output("Output file not collected properly\n");
            pending_files->push_back((*work_table)[key]);
            work_table->erase(key);
            if(read_size==0 || read_size==-1){
                client_table->erase(key);
            ...
            log_output("Output file collected succesfully from Client %d!\n", key);
...
void udp_update_broadcast(int flag, std::map<int, client_info> *client_table, std::vector<sockaddr_in> *next_servers){
    if(flag==1){
        int serverfd = socket(AF_INET, SOCK_DGRAM, 0);
        sockaddr_in server_addr, client_addr;
        if(serverfd<0){
            log_output("Server for UDP broadcast couldn't be created.\n");
            return ;
        }
...
void* start_server(void *params){
    ...
    std::vector<std::string> *pending_files = p->pending_files;
    std::vector<std::string> *completed_files = p->completed_files;
    std::set<unsigned int> *priority_table = p->priority_table;
    std::map<int, client_info> *next_servers = p->next_servers;
    int port = p->port;
    ...
    log_output("Sever Socket has been binded to the port : %d\n",port);
    ...
    while( (new_socket = accept(socket_desc, (struct sockaddr *)&client, (socklen_t*)&c)) ){
        ...
        if(next_servers->size() < BACKUP_SERVERS){
            ...
            pc->replicate = 1 + size;
            next_servers->insert(std::pair<int, client_info> (size,*c));
            // pc->replicate = 1 + size();
        }
...
void* distribute_work(void* params){
        ...
        for(auto x: *pending_files){ // iterate through the pending files list
            for(rit = priority_table->rbegin(); rit!=priority_table->rend(); rit++){ //get available client for the pending file by iterating through the client table and checking if a given client is busy
                ...
                if(!((*client_table)[key].busy) && x.length() > 0){ //if client is free assign pending file to it
                    ...
                    log_output("Work has been assigned to client %d : %s\n", key, x.c_str());
    ...
    log_output("All the give tasks in the so_files folder has been executed successfully!\n");
}
...
int return_specs(){
    const char *command = "cat /proc/cpuinfo | grep MHz > output.txt";
    system(command);
    std::fstream file;
    std::string word;
...
int replicate_state(int clientSocket, std::map<int, client_info_cs>* next_servers, int* prio){
    //tab begin
    int file_nos = 0;
    char temp[25];
    ...
    if(strcmp(temp,"file") == 0){
        ...
        log_output("This client has been elected as a backup server!\n");
        if(mkdir("../so_files",0777)==-1){
            log_output("The so_files directory could not be created!.\n");
        }
        if(mkdir("../op_files",0777)==-1){
            log_output("The op_files directory could not be created!.\n");
        }
        read(clientSocket, &file_nos, sizeof(int));
        for(int i=0;i<file_nos;i++){
            recv_file_cs(clientSocket, ("./so_files/"+std::to_string(i)+".so").c_str() );
        }
        log_output("State replication completed succesfully!.\n");
        read(clientSocket, temp, 5); //this reads for the nstb command
    }

    assert(strcmp(temp, "nstb")==0);
    log_output("Accepting backupserver details from current server.\n");
    int client_nos = 0;
    ...
    for(int i = 0; i<client_nos; i++){
        ...
        next_servers->insert(std::pair<int, client_info_cs> (order, c));
    }
    log_output("Backups Server table updated.\n");
    log_output("Backup server details are as follows....\n");
    for(auto x : *next_servers){
        log_output("Backup server %d : %s:%d\n", x.first, x.second.ipAddr, x.second.port);
    }
    return 1;
}


void handle_server_failure(int prio, int port, std::map<int,client_info_cs>* next_servers, client_ret_details* crd){
    if(prio==0){
        crd->server = 1;
        crd->port = port+1;
    }
...
client_ret_details* set_up_client(char* addr, int port){
    ...
    char buffer[BUFFER_LEN];
    std::map<int, client_info_cs> next_servers;
    client_ret_details* crd = (client_ret_details*)malloc(sizeof(client_ret_details));

    log_output("This is the ClusterCreate Client programme.\n");
    log_output("Setting up connection with the server...\n");
    ...
    while (1){
        ...
        if(t==-1 || t==0){ //it actually return 0
            log_output("Connection to server lost!.\n");
            // exit(1);
...
void runclient(const char* addr, int port){
    setbuf(stdout, NULL);
    client_ret_details* crd = (client_ret_details*)malloc(sizeof(client_ret_details));
    crd = set_up_client(const_cast<char *>(addr), port);
    while(1){
        if(crd->server){
            runserver(crd->port);
        }
